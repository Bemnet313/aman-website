  ...
  return (
    <section className="w-full py-48 px-10 md:px-32 bg-[#0d2c2c] relative z-20 flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 w-full flex flex-col items-center">
        <motion.h2 
          ref={ref}
          className="text-4xl md:text-7xl lg:text-8xl font-bold tracking-[0.1em] text-center mb-32 text-white"
        >
          Embrace Your True Self
        </motion.h2>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-16 md:gap-32 w-full max-w-6xl text-center">
          <StatItem value="9.5M+" label="YouTube Views" isInView={isInView} delay={0.2} />
          <StatItem value="2" label="Award Nominations" isInView={isInView} delay={0.4} />
          <StatItem value="14" label="Began Producing at Age" isInView={isInView} delay={0.6} />
        </div>
      </div>
    </section>
  );
